...
Paper Mode V1 itself remains a paper-trading release candidate. Post-v1 live work now includes constrained TESTNET `MARKET` / `LIMIT` order placement, cancel, cancel-all-active-symbol, flatten, closed-candle auto-execution, kill switch controls, env-backed local credential loading, default-off manual MAINNET canary execution, MAINNET auto dry-run infrastructure, gated MAINNET auto live-session support for explicit bounded runs, explicit cross/isolated margin gating, and ASO position policy modes. The current mainnet-canary phase closed after two bounded non-marketable `LIMIT` canaries submitted, canceled, reconciled flat, passed restart repair, and disabled the canary flag afterward. The cancel endpoint ergonomics issue found in the second canary was fixed. The first 15-minute MAINNET auto live trial completed on 2026-04-25 with no signal, no order, no fill, watchdog stop `max_runtime_reached`, and final flat state. MAINNET auto live execution remains disabled by default. This does not change the Paper Mode V1 scope.
