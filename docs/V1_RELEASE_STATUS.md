...
Paper Mode V1 itself remains a paper-trading release candidate. Post-v1 live work now includes constrained TESTNET `MARKET` / `LIMIT` order placement, cancel, cancel-all-active-symbol, flatten, closed-candle auto-execution, kill switch controls, env-backed local credential loading, and default-off manual MAINNET canary execution. The latest MAINNET reference-price-fixed canary submitted one non-marketable `LIMIT` order, canceled it cleanly, reconciled flat, passed restart repair, and disabled the canary flag afterward. This does not change the Paper Mode V1 scope.
