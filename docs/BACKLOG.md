...
- Turkish friend/new-reviewer handoff guide added and expanded in `docs/FRIEND_HANDOFF.md` and linked from `README.md`, covering architecture, implemented capabilities, setup, build/test/run commands, API payload examples, TESTNET evidence, MAINNET canary/auto live command shapes, and the current next task.
