...
Mainnet Auto Live Support v1 is implemented as a gated session-scoped path. `POST /api/live/mainnet-auto/start` requires server live config, `RELXEN_MAINNET_AUTO_MODE=live`, `BTCUSDT`, `MARKET`, an explicit runtime (`15`, `60`, or `0` for operator-stop), the exact session confirmation for that runtime such as `START MAINNET AUTO LIVE BTCUSDT 15M` or `START MAINNET AUTO LIVE BTCUSDT OPERATOR STOP`, fresh live gates, risk budget, watchdog, and evidence/lesson logging. Mainnet Auto Policy Support v1 adds explicit margin-type gating and ASO position policy modes: allowed margin type defaults to `isolated`, actual `cross` must be explicitly allowed, actual `unknown` blocks live auto, and ASO policy defaults to `crossover_only` with optional `always_in_market` and `flat_allowed` modes. The follow-up hardening adds mocked support for `always_in_market` reverse and flat-stop through reduce-only close plus flat reconciliation, while `crossover_only` preserves its conservative open-position blocker. The operator helper accepts the explicit live-trial flags, including max leverage up to `100x`, and verifies the running server is already session-scoped for live auto before calling the existing start endpoint. MAINNET auto remains disabled by default.
...
- `RELXEN_MAINNET_AUTO_MAX_RUNTIME_MINUTES`, `RELXEN_MAINNET_AUTO_MAX_ORDERS`, `RELXEN_MAINNET_AUTO_MAX_FILLS`, `RELXEN_MAINNET_AUTO_MAX_NOTIONAL`, `RELXEN_MAINNET_AUTO_MAX_DAILY_LOSS`: typed mainnet-auto session budget inputs. The planned live trial shape uses 15 minutes, emergency order/fill caps of 20, notional cap 80, loss cap 5, and max leverage no higher than the explicit budget with a hard cap of 100.
...
- `RELXEN_MAINNET_AUTO_ALLOWED_MARGIN_TYPE`: `isolated`, `cross`, or `any`; default `isolated`. Unknown margin type always blocks live MAINNET auto.
- `RELXEN_MAINNET_AUTO_POSITION_POLICY`: `crossover_only`, `always_in_market`, or `flat_allowed`; default `crossover_only`.
- `RELXEN_MAINNET_AUTO_ASO_DELTA_THRESHOLD`, `RELXEN_MAINNET_AUTO_ASO_ZONE_THRESHOLD`: conservative filters for `flat_allowed`.
...
- Live MAINNET auto-execution by default. The gated support path exists and one no-order live trial completed, but a separate explicit execution task is still required for any repeat run.
